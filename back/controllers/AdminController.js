...
var HistorialProducto =require('../models/HistorialPrecioStock');
...
const actualizar_producto_admin = async function(req,res){
    if(req.user){
        ...
        if(req.files){
            ...
            await HistorialProducto.create({
                productoId: reg._id,
                precio: reg.precio, // Ajusta según tus datos
                stock :req.stock,
                titulo: reg.titulo, // Ajusta según tus datos
                sku: reg.sku, // Ajusta según tus datos
            });
...
const actualizar_config_admin = async (req,res)=>{
    if(req.user){
        ...
        let config = await Config.findByIdAndUpdate({_id:'61abe55d2dce63583086f108'},{
            ...
            monto_min_pesos: data.monto_min_pesos,
...
const registro_compra_manual_cliente = async function(req,res){
    if(req.user){
       ...
       await enviar_orden_compra(venta._id);
