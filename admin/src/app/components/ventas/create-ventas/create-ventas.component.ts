...
export class CreateVentasComponent implements OnInit {
      ...
      response=>{
          ...
          response.data.forEach((element:any) => {
              this.variedades.push({
                ...
                precio_pesos: element.producto.precio,
      ...
      if(this.cantidad >= 1){
        if(this.cantidad <= this.producto_select.cantidad){
          this.dventa.push({
            ...
            precio_und: this.producto_select.precio_pesos,
            titulo_variedad: this.producto_select.variedad,
            producto: this.producto_select.idproducto,
            subtotal: this.producto_select.precio_pesos * this.cantidad,
            variedad: this.producto_select.idvariedad,
            cantidad: this.cantidad
          });
          console.log( this.dventa);
          this.total_pagar = this.total_pagar + (this.producto_select.precio_pesos * this.cantidad);
          this.neto_pagar = this.neto_pagar + (this.producto_select.precio_pesos * this.cantidad);
        }else{
          iziToast.show({
              title: 'ERROR',
              titleColor: '#FF0000',
  ...
  calcular_envio(){
    ...
    if(this.direccion_select.pais != 'Colombia'){
    ...
    }else if(this.direccion_select.pais == 'Colombia'){
      if(this.direccion_select.region == 'Bogotá'){
        this.envio_input = 10;
      }else if(this.direccion_select.region != 'Bogotá'){
        this.envio_input = 15;
      }
    }
